...
Les méthodes ensemblistes désignent les approches qui consistent à entraîner plusieurs modèles, puis à combiner les résultats obtenus afin de produire une prédiction consolidée. Les performance des méthodes ensemblistes sont souvent supérieures à celles des modèles individuels.

Les modèles utilisés dans les méthodes ensemblistes, appelés modèles de base ou "apprenants faibles" ("weak learners"), sont généralement peu complexes. Le choix de ces modèles et la manière dont leurs prédictions sont combinées sont des facteurs clés pour la performance de ces approches.

Les méthodes ensemblistes peuvent être divisées en deux grandes familles selon qu'elles s'appuient sur des modèles entrainés en parallèle ou de manière imbriquée. Dans le premier cas, chaque modèle de base est entraîné pour la même tâche de prédiction en utilisant un sous-ensemble de l'échantillon d'entraînement, un sous-ensemble de variables, ou une combinaison des deux. Les techniques les plus populaires sont le bagging et la forêt aléatoire. Dans la seconde approche, appelée boosting, les modèles de base sont entraînés de manière séquentielle, chaque modèle visant à minimiser l'erreur de prédiction du modèle précédent. Les implémentations les plus courantes du boosting sont actuellement XGBoost, CatBoost et LightGBM.
...
La Random Forest est un algorithme d'apprentissage ensembliste basé sur le bagging (Bootstrap Aggregating) qui combine les prédictions de multiples arbres de décision pour améliorer les performances de prédiction et réduire le surapprentissage. 

Etapes de construction du modèle :

    Sous-échantillonnage bootstrap :
        Pour chaque arbre de décision dans la forêt, un échantillon bootstrap est généré à partir de l'ensemble d'entraînement initial. Cela signifie qu'un échantillon est tiré au hasard avec remplacement à partir des données d'entraînement, ce qui crée un nouvel ensemble de données de la même taille mais avec des exemples potentiellement dupliqués.

    Construction de l'arbre de décision :
        Sur la base de chaque échantillon bootstrap, un arbre de décision est construit. À chaque nœud de l'arbre, un sous-ensemble aléatoire de caractéristiques est considéré pour le split.
        L'arbre est construit en divisant récursivement les données en sous-ensembles homogènes jusqu'à ce qu'un critère d'arrêt soit atteint, tel que la profondeur maximale de l'arbre ou le nombre minimum d'exemples par feuille.

    Entraînement de la forêt :
        Une fois que tous les arbres de décision sont construits, les prédictions de chaque arbre sont agrégées. Pour une classification, le mode (classe la plus fréquente) est utilisé comme prédiction finale, tandis que pour une régression, la moyenne des prédictions est prise.
...
Etapes de construction du modèle :
    ...
    Construction du modèle de base :
        Le modèle de base accorde une importance (poids) plus élevée aux exemples mal classés par les modèles précédents.
