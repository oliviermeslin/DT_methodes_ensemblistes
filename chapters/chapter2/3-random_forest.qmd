...
- **Si $\rho(x)$ est élevée** : Les arbres sont plus corrélés entre eux et la réduction de variance est moindre lorque le nombre d'arbres augmente.
...
- **Nombre minimal d'observations pour diviser un nœud** : Empêche la création de nœuds avec très peu d'observations, ce qui peut améliorer la généralisation des prédictions. 
...
Objectif: Identifier les variables qui contribuent le plus à la prédiction, c'est-à-dire qui influencent le plus la variable cible, extraire des caractéristiques pertinentes pour comprendre les mécanismes de prédiction sous-jacent, établir des règles de décision simplifiées etc.
...
- Utiliser des méthodes d'interprétabilité avancées : SHAP values, LIME (?)
