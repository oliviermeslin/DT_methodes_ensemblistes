...
- **Nombre de variables considérées à chaque noeud**: cet hyperparamètre détermine le nombre de variables prédictives sélectionnées aléatoirement à chaque nœud lors de la construction des arbres.

Un nombre plus faible de variables candidates conduit à des arbres plus diversifiés et donc moins corrélés entre eux. Cette réduction de la corrélation entre les arbres est bénéfique pour l'agrégation finale, car elle permet de réduire la variance globale du modèle, améliorant ainsi sa capacité de généralisation. Cependant, cela peut entraîner une diminution de la qualité prédictive des arbres individuels, car ils sont parfois contraints de se diviser sur des variables moins pertinentes.
...
La méthode SMBO (Sequential model-based optimization) est une approche plus efficace que les précédentes car elle s'appuie sur les résultats des évaluations déjà effectuées pour guider la recherche des prochains hyper-paramètres à tester. 
...
Les forêts aléatoires sont des modèles d'apprentissage performants mais difficiles à interpréter, souvent qualifiés de "boîtes noires". Comprendre l'influence de chaque variable explicative sur la prédiction est crucial pour l'interprétation. Dans cet objectif, plusieurs mesures d'importance des variables ont été développées.
...
- **Réduction moyenne de l'impureté** (_Mean Decrease in Impurity_ ou _MDI_) : pour chaque variable, on calcule la moyenne des réductions d'impureté qu'elle a engendrées dans tous les nœuds de tous les arbres où elle est impliquée. Les variables présentant la réduction moyenne d'impureté la plus élevée sont considérées comme les prédicteurs les plus importants.

- **Permutation importance** (_Mean Decrease Accuracy_ ou _MDA_) : Pour chaque variable, les performances du modèle sont comparées avant et après la permutation de ses valeurs. La différence moyenne de performance correspond à la MDA. L'idée est que si l'on permute aléatoirement les valeurs d'une variable (cassant ainsi sa relation avec la cible), une variable importante entraînera une hausse significative de l'erreur de généralisation. Il existe de nombreuses implémentations de la MDA que l'on retrouve dans les différents packages usuels en R et en Python. Néanmoins, la MDA peut être biaisée lorsque les variables sont corrélées, ce qui conduit au classement erroné des variables par ordre d'importance @benard2022mda.
...
- **Valeurs de shapley** (_Shapley values_): Les valeurs de Shapley permettent de quantifier la contribution de chaque variable explicative à la variance expliquée de la variable cible, en tenant compte des interactions entre les variables. Cependant, leur estimation est computationnellement coûteuse (exponentielle avec le nombre de variables) et pose des défis méthodologiques. La plupart des méthodes /implémentations présentent soit des biais du fait de l'usage d'approximations, soit une complexité computationnelle élevée (en entraînant un modèle pour chaque sous-ensemble de variables). La méthode SHAFF (@benard2022shaff), implémentée en R, permet toutefois de pallier à ces problèmes: elle offre une estimation rapide et sans biais des valeurs de Shapley.
...
- Utiliser des méthodes d'interprétabilité avancées alternatives : La Sobol-MDA, 


**Méthodes recommandées**: 

- **Conditional Inference Forests** (CIF) : Au lieu des implémentations classiques de la MDI, @strobl2007bias propose d'utiliser des forêts aléatoires basées sur l'inférence conditionnelle, implémentées dans le package party de R (algotithme cforest). Les CIF corrigent le biais de sélection des variables en utilisant des tests statistiques conditionnels pour choisir les variables et les points de coupure. Couplé à un échantillonnage sans remise, cette méthode permet d'obtenir des mesures d'importance des variables fiables et non biaisées, même avec des variables hétérogènes.

- **Sobol-MDA** : La Sobol-MDA vise à corriger les limites de la MDA en fournissant une mesure d'importance cohérente, même en présence de variables dépendantes. Au lieu de permuter les valeurs des covariables, la Sobol-MDA modifie la prédiction des arbres en ignorant les divisions basées sur la covariable cible. Elle utilise un algorithme "Projected-CART" pour projeter la partition de l'espace des covariables sur le sous-espace sans la covariable cible. Cela permet de simuler la suppression effective de cette variable. Elle converge asymptotiquement vers l'indice de Sobol total, la mesure appropriée pour identifier les covariables les plus influentes, même avec des dépendances. De plus, elle est beaucoup plus efficace en termes de calcul que les méthodes alternatives utilisant la MDA.
