...
Les forêts aléatoires nécessitent généralement moins d’optimisation que d’autres modèles d’apprentissage. En effet, leurs performances varient relativement peu en fonction des hyperparamètres. Les valeurs par défaut fournissent souvent des résultats satisfaisants, ce qui réduit le besoin d'optimisation intensive. Cependant, un ajustement précis des hyperparamètres peut apporter des gains marginaux mais utiles, notamment sur des jeux de données complexes.
...
Ce compromis est atteint en ajustant certains hyperparamètres clés comme le nombre de variables candidates à chaque division, la taille de l'échantilon ou encore la taille minimale des feuilles.

L'ajustement du nombre d'arbre relève plutôt d'un arbitrage entre performance et temps de cacul. Il est recommandé de fixer le nombre d'arbre à une valeur au-delà de laquelle les gains de performance peuvent être considérés comme marginaux.
...
- Débuter avec les valeurs par défaut: entraîner une forêt aléatoire avec les valeurs des hyperparamètres par défaut. 
- Ajuster le nombre d’arbres: augmenter progressivement le nombre d'arbres jusqu'à atteindre une performance stable. Fixer le nombre d'arbres à cette valeur par la suite.
- Explorer rapidement les hyperparamètres principaux: utiliser les méthodes _grid search_ (pour explorer toutes les combinaisons possibles dans un espace discret) ou _random search_ (pour explorer aléatoirement un espace plus large). Outils recommandés : `caret` (R) ou `scikit-learn` (Python).
- Évaluer avec l’erreur Out-of-Bag (OOB) : utiliser l'erreur OOB pour une évaluation rapide offre et fiable des performances du modèle en fonction des différents hyper-paramètres (sans nécessiter de cross-validation).
- Validation finale du modèle optimisé: comparer les performances finales sur un jeu de test ou par une validation croisée.
...
On conseil l'utilisation de trois implémentations pour comparer l'importances des variables d'une forêt aléatoire:
