...
- Adopter des approches basées sur SMBO si les performances initiales sont insuffisantes ou si l’espace est complexe. En `R`, il existe plusieurs implémentations d'appuyant sur ces méthodes: tuneRF (limité à l’optimisation de mtry), tuneRanger (optimise simultanément mtry, node size, et sample size).
