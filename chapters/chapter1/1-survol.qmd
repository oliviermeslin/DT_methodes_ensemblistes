...
Plutôt que de s'appuyer sur un seul modèle complexe, les méthodes ensemblistes combinent les prédictions de plusieurs modèles plus simples, appelés "apprenants faibles" (_weak learner_ ou _base learner_), pour créer un "apprenant fort" plus performant. 

Le choix de ces modèles de base, ainsi que la manière dont leurs prédictions sont combinées, sont des facteurs déterminants de la performance de ces approches. Le présent document se concentre sur les méthodes à base d'**arbres de décisions**, qui sont parmi les plus utilisées en pratique. Nous allons examiner les fondements de ces méthodes, leurs avantages et inconvénients, ainsi que les algorithmes les plus populaires.
