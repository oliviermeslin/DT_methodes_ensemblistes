...
Le surapprentissage (_overfitting_) désigne le fait qu'un algorithme complexe et flexible a tendance à refléter trop précisément les données sur lesquelles il a été entraîné, c'est-à-dire à produire un modèle qui donne en apparene de très bons résultats pour ces données, mais qui serait probablement nettement moins performant sur d'autres données non utilisées pendant l'entraînement. On dit d'un modèle souffrant de surapprentissage qu'il a une faible capacité de généralisation, car les relations statistiques qu'il a extraites des données d'entraînement ne sont pas valables pour d'autres jeux de données.
:::
