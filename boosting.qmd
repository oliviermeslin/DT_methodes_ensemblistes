...
### Les premières approches du _boosting_ : Adaboost et la _Gradient Boosting Machine_
...
Le principe d'Adaboost consiste à pondérer les erreurs commises à chaque itération en donnant plus d'importance aux observations mal prédites. Voici une esquisse du fonctionnement d'AdaBoost:
...
La _Gradient Boosting Machine_ propose une approche assez différente: elle établit un lien entre le _boosting_ et l'utilisation d'une fonction de perte.
...
@friedman2001greedy
fait le lien entre le _boosting_ et l'utilisation d'une fonction de perte.
...
### La modélisation additive par étapes

La modélisation additive par étapes parvient à approximer la fonction FFF en ajoutant progressivement des modèles simples ffffff sans modifier les modèles simples qui ont déjà été entraînés. A chaque itération, on recherche la fonction $f(x, theta_m)$ qui améliore le plus possible la fonction $f_(m-1)(x)$. On obtient alors la fonction $f_m(x)$, et la procédure recommence.

1.  Initialiser `#mi("$f_0(\mathbf{x}) = \delta$")`{=typst}.
2.  Pour `#mi("$m = 1, \dots, M:$")`{=typst}
    (a) Entraîner le $m$-ième modèle:
    ```{=typst} 
    mitex(`$(\hat{\beta}_m, \hat{\theta}_m) = \argmin_{\beta, \mathbf{\theta}} \sum_{i=1}^N L(y_i, f_{m-1}(\mathbf{x}_i) + \beta b(\mathbf{x}_i, \mathbf{\theta}))$
    `)
    ```

    (b) Définir `#mi("$f_m(\mathbf{x}) = f_{m-1}(\mathbf{x}) + \hat{\beta}_m b(\mathbf{x}_i, \mathbf{\hat{\theta}_m})$")`{=typst}
