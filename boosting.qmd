...
Si elles peuvent sembler complexes au premier abord, ces deux formules s'avèrent très utiles en pratique pour deux raisons. Tout d'abord, la formule `@w_j_optimal`{=typst} permet de calculer très rapidement les poids d'un arbre donné, ce qui contribue aux performances des approches de _gradient boosting_. Ensuite et surtout, on peut utiliser directement la formule `@fct_obj_optimal`{=typst} pour comparer deux arbres, et donc pour évaluer le gain induit par un _split_ marginal. Imaginons qu'on envisage de décomposer la feuille $I$ en deux nouvelles feuilles $I_L$ et $I_R$ (avec $I = I_L \cup I_R$). Le gain potentiel induit par ce _split_ est égal à:
