...
```
| Hyperparamètre | Description | Commentaire |
|------------------------|------------------------|------------------------|
| `booster` | Le type de weak learner utilisé | `gbtree` |
...
| `min_child_weight` | Le poids |  |
| `num_estimators` | Le nombre d'arbres |  |
| `lambda` | La pénalisation L2 | 1 |
| `alpha` | La pénalisation L1 | 0 |
...
| `subsample` | Le taux d'échantillonnage des données d'entraîement | 1 |
...
: Les hyperparamètres d'XGBoost {tbl-colwidths="\[20,30,50\]"}
...
```{=typst} 
#mitex(`
$obj^{(k)} = \sum_{i=1}^n (y_{i} - (\hat{y}_{i}^{(k-1)} + f_t(x_i)))^2 + \sum_{i=1}^t\Omega(f_i) \\ = \sum_{i=1}^n [2(\hat{y}_i^{(k-1)} - y_i)f_t(x_i) + f_t(x_i)^2] + \Omega(f_t) + constant$
`)
```

```{=typst} 
#mitex(`
$obj^{(k)} = \sum_{i=1}^{n} [l(y_i, \hat{y}_i^{(k-1)}) + g_i f_t(x_i) + \frac{1}{2} h_{i} f_{t}^2(x_i)] + \Omega(f_t) + constant$
`)
```
...
```{=typst} 
#mitex(`
$\Omega(f) = \gamma T + \frac{1}{2}\lambda \sum_{j=1}^T w_j^2 obj^{(k)} \approx \sum_{i=1}^n [g_i w_{q(x_i)} + \frac{1}{2} h_i w_{q(x_i)}^2] + \gamma T + \frac{1}{2}\lambda \sum_{j=1}^T w_j^2\\ = \sum^T_{j=1} [(\sum_{i\in I_j} g_i) w_j + \frac{1}{2} (\sum_{i\in I_j} h_i + \lambda) w_j^2 ] + \gamma T$
`)
```
...
```{=typst} 
#mitex(`
$obj^{(k)} = \sum^T_{j=1} [G_jw_j + \frac{1}{2} (H_j+\lambda) w_j^2] +\gamma T$
`)
```
