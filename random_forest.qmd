...
Le tableau @tbl-hyp-rf présente les principaux hyperparamètres des forêts aléatoires. Le nom des hyperparamètres est celui figurant dans le _package_ `R` `ranger`. Il arrive qu'ils portent un nom différents dans d'autres implémentations des _random forests_, mais il est généralement facile de s'y retrouver en lisant attentivement la documentation.
...
: Les principaux hyperparamètres des forêts aléatoires {#tbl-hyp-rf tbl-colwidths="[25,80]"}
